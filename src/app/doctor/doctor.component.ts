...
export class DoctorComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this._appService.getParticipant(this._appService.getUsername(), Participants.Doctor)
      .subscribe(data => {
        let p: any = data;
        console.log(p.length ? "y" : "n");

        if (p.length) {
          this.doctorId = p[0]._id;
        } else {
          console.error("doctorId !!!");
          return;
        }
      },
        error => {
  ...
  getRecords() {
    this._appService.getRcords(this.familyUsername, Participants.Doctor)
      .subscribe(data => {
